...
"""
================================================================================
CRYPTO BACKTESTER PRO v5.0 - UNIVERSAL DATA + LIVE STREAMING
- Multi-source data: Binance, CoinGecko, CryptoCompare, Synthetic fallback
- Live price monitoring with auto-refresh
- Extended historical periods (up to 2000 candles)
- 20 crypto pairs
- Fixed all Streamlit and pandas issues
================================================================================
"""
...
class BacktestEngine:
    ...
    def run(self,tf='1h',entry=None,exit=None,sl_atr=2,tp_atr=3,use_sl=True,use_tp=True,use_trail=False,trail_atr=2,start=None,end=None):
        ...
        for i,(ts,row) in enumerate(df.iterrows()):
            ...
            if self.pos and exit and eng.eval_rule(exit,ts): self.exit(ts,row['close'],'signal_exit'); self.update_eq(ts,row['close']); continue
            if not self.pos and entry and eng.eval_rule(entry,ts): self.enter(ts,row['close'],'long',sl_atr,tp_atr,use_sl,use_tp,use_trail,trail_atr)
...
def render_header():
    ...
    st.markdown('<div class="subtitle">Real Data | Multi-Timeframe | Multi-Pair | Live Stream</div>', unsafe_allow_html=True)
...
def main():
    ...
    with tab1:
        ...
        else: entry_rule=entry_conditions[0]
    ...
    st.markdown("""<div class="footer">Crypto Backtester Pro v5.0 | Universal Data Sources | Live Streaming | Built with Streamlit</div>""",unsafe_allow_html=True)
