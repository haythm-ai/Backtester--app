...
"""
================================================================================
CRYPTO BACKTESTER PRO - STREAMLIT WEB APP v2.0
Fixed for Streamlit Cloud + Latest Pandas
================================================================================
"""
...
# ============================================================
# DATA GENERATION - FIXED FOR ALL PANDAS VERSIONS
# ============================================================

def generate_data():
    """Generate synthetic crypto data - compatible with all pandas versions"""
    np.random.seed(42)
    end = datetime(2026, 5, 15, 22, 0)

    # Use lowercase 'h' for hours (compatible with pandas 2.0+)
    # Generate 90 days of hourly data
    n_hours = 90 * 24

    # Create datetime index manually to avoid freq string issues
    dates_1h = [end - timedelta(hours=i) for i in range(n_hours)]
    dates_1h.reverse()  # Oldest first
    ...
    }, index=pd.DatetimeIndex(dates_1h))
...
def render_metric_card(label, value, suffix="", is_pct=False):
    color = "positive" if (is_pct and float(value) > 0) else "negative" if (is_pct and float(value) < 0) else ""
    st.markdown(f"""
    <div style="background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); padding: 1rem; border-radius: 10px; color: white; margin: 0.3rem 0;">
...
def main():
    ...
    entry_logic = st.selectbox(
        "How should entry conditions combine?",
        ['AND (All must be true)', 'OR (Any can be true)', 'NOT (None should be true)'],
        key="entry_logic_select"
    )
    entry_logic_clean = entry_logic.split(' ')[0]  # Extract AND, OR, or NOT

    # Exit Logic Selection  
    st.subheader("Exit Logic")
    exit_logic = st.selectbox(
        "How should exit conditions combine?",
        ['OR (Any can trigger exit)', 'AND (All must trigger)', 'NOT (None should trigger)'],
        key="exit_logic_select"
    )
    exit_logic_clean = exit_logic.split(' ')[0]  # Extract OR, AND, or NOT

    entry_rule = {'logic': entry_logic_clean, 'conditions': entry_conditions}
    exit_rule = {'logic': exit_logic_clean, 'conditions': exit_conditions}
    ...
    st.markdown("---")
    st.markdown("<div style='text-align: center; color: #999; font-size: 0.8rem;'>Crypto Backtester Pro v2.0 | Mobile Optimized</div>", 
                unsafe_allow_html=True)
