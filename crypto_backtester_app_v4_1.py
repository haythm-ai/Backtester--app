...
"""
================================================================================
CRYPTO BACKTESTER PRO v4.0 - REAL DATA EDITION
...
- Multi-pair support with real crypto data
================================================================================
"""
...
def fetch_binance_klines(symbol, interval, limit=1000, days_back=None):
...
def build_condition_card(prefix, key_prefix, index):
    ...
    with st.container():
        ...
        # Row 1: Timeframe | Indicator | Operator
...
def main():
    ...
    with tab1:
        ...
        # NOTE: data_limit is already in session_state via its widget key="data_limit"
        # Do NOT assign st.session_state.data_limit here — that raises StreamlitAPIException
    ...
    with tab3:
        if 'run_backtest' not in st.session_state or not st.session_state.run_backtest:
            ...
            selected_pairs = st.session_state.get('selected_pairs', ['BTC/USDT'])
            data_limit = st.session_state.get('data_limit', 500)
    ...
    with tab4:
        if 'results' not in st.session_state or st.session_state.results is None:
            ...
            if not trades_df.empty:
                ...
                # Add pair column if available
                if 'pair' not in trades_df.columns and len(all_results) > 1:
                    # Reconstruct pair column from per-pair trades
                    trades_with_pair = []
                    for r in all_results:
                        tdf = r['trades'].copy()
                ...
                try:
                    ...
                    styled = display_df.style.applymap(color_pnl, subset=['P&L ($)', 'P&L (%)'])
